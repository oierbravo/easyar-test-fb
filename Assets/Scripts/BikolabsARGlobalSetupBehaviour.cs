...
namespace Sample
{
    public class BikolabsARGlobalSetupBehaviour : MonoBehaviour
    {
    private DatabaseReference mDatabaseRef;
        ...
        void OnTargetFound(ARCameraBaseBehaviour arcameraBehaviour, TargetAbstractBehaviour targetBehaviour, Target target)
        {
           ...
           string key = mDatabaseRef.Child("events").Push().Key;
            mDatabaseRef.Child("events").Child(key).Child("text").SetValueAsync("target: " + target.Name + " found");
           mDatabaseRef.Child("events").Child(key).Child("targetName").SetValueAsync(target.Name);
    }
